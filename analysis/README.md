...
```train
python simulations/sim_motorcircle_basic.py pt 0 3 100
```

The example `sim_motorcircle_basic` above runs the data simulation of a motor decision task (Kurniawan 2010) with given input parameters, fitted the sumulated data hierarchically using Stan and produce the parameter distribution in output sample traces. It has the following changable parameters:
...
python simulations/sim_motorcircle_basic.py pt $sim_num 70 240
python simulations/sim_motorcircle_basic.py hc $sim_num 70 240
done
```
...
```eval
python visualisation/hdi_compare.py 160 3 motorcircle 2 50
```
Where the first argument is the number of trials used in the simulation (160), the second argument is the number of subjects in the simulation (3), the third argument corresponds to model name (sim_motorcircle_basic), the fourth specifies the number of simulations minus 1, and the last number of permutations.
...
```eval
python data_transform/convert_data.py circlemotor ../../task 100
```

The task name is `circlemotor`, which is specified in argument 1 above. The 2nd argument is the relative path of your Pavlovia task directory (forked from our source, cloned to your local machine). Alternatively, you can write your own data conversion function to match any changes you've made to the task. Finally, the last optional argument specifies how many trials to expect in the dataset for each participant to reject files which are not complete, if not specified, incomplete files will be included.
...
```eval
python data_fit/fit_motorcircle_basic.py A,B
```
